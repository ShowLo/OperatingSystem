...
{
	rb->u1.Parent = (PMMADDRESS_NODE)(rb_color(rb) + (LONG_PTR)parent);
}
    ...
    )
{
	...
	/*
	*如果NodeToDelete有任一空子节点，则记录child并跳转至条件语句后的语句
	*如果两子节点都不空，则进入else分支
	*/
	...
	else
	{
		...
		*先找到它的右子节点，再循环找左子节点
		*知道没有左子节点，找到的节点一定是比NodeToDelete大且紧邻NodeToDelete的节点
		*将找到的此节点放到NodeToDelete位置，并保持NodeToDelete的原有颜色，树的性质不受影响
		...
		*/
		...
		/*
		*找到节点之后，将其转移过去
		*首先用old的父节点或Table指向新找到的节点NodeToDelete
		*/
		if (rb_parent(old)) {
		...
		/*
		*记录找到的NodeToDelete节点的信息，对于这种删除操作，NodeToDelete节点移动至old位置后会沿用old的颜色
		*树的性质不受影响，但原NodeToDelete的位置将失去一个节点，需要记录其信息以便做删除
		*/
		child = NodeToDelete->RightChild;
		...
		if (parent == old) {
			...
			*这里保存的parent可以理解为将用作原来子节点的亲父节点（对于old是NodeToDelete右子节点的情况）
			*NodeToDelete取代old后，原NodeToDelete的父节点还是NodeToDelete
			*/
			parent = NodeToDelete;
		} else {
			/*
			*NodeToDelete节点的删除处理，主要是将相应的父、子给连接起来
			*删的数据是old节点，但将NodeToDelete转移过去占了old的位置
			*所以对于节点结构来说，删除的是NodeToDelete原来的位置
			*/
			if (child)
		...
		/*
		*NodeToDelete转移过去之后继续使用old节点的颜色和父节点属性
		*/
		NodeToDelete->u1.Parent = old->u1.Parent;
		/*
		*之前已经将old的parent的指向old子节点的指针指向了NodeToDelete
		*并且NodeToDelete也已经连接了old的右子节点（如果NodeToDelete与old是子与父的关系，则NodeToDelete保留原右子节点即可）
		*已经完成NodeToDelete指向新父节点即设置颜色的操作，最后只剩下左子节点了
		*/
	...
	/*
	*这里对应于NodeToDelete有任一空子节点即NodeToDelete无子节点或者有单子节点的情况
	*区别是NodeToDelete可能是右子节点也可能是左子节点
	*/
	parent = rb_parent(NodeToDelete);
	color = rb_color(NodeToDelete);
    ...
    )
{
    ...
    TABLE_SEARCH_RESULT result = MiFindNodeOrParent(Table,
    NodeToInsert->StartingVpn,&NodeOrParent);

    //先设置插入节点的右子节点和左子节点均为空节点
    NodeToInsert->LeftChild = NULL;
	NodeToInsert->RightChild = NULL;
    ...
    else
    {
        ...
        //接下来利用Linux的rb_insert_color进行树的维护
